...
const communityLinks = computed(() => [{
  ...
  label: '编辑此页',
  to: `https://github.com/pacmandoh/PacDocs/edit/main/docs/${page?.value?._file?.split('/').slice(1).join('/')}`,
  target: '_blank'
}, {
  ...
  icon: 'i-mdi-mail-ru',
  label: '邮件我',
  to: 'mailto:pacmandoh@outlook.com',
  target: '_self'
}])
...
<template>
  ...
  >
    ...
    <template v-if="page.toc !== false" #right>
      <UContentToc :links="page.body?.toc?.links" title="目录" :ui="{ wrapper: '' }">
        <template #bottom>
          <div class="hidden lg:block space-y-6" :class="{ '!mt-6': page.body?.toc?.links?.length }">
            ...
            <UPageLinks title="生态" :links="ecosystemLinks" />
